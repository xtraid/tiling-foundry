...
from dossier.tex_compile import TexCompileError, compile_tex_pdf  # noqa: E402
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--pdf",
        action="store_true",
        help="compile the additive static PDF for v2 cases only",
    )
    return parser
...
def _compile_pdf(dossier_root: Path, tex_engine: str, captured_at: datetime) -> None:
    try:
        compile_tex_pdf(dossier_root, tex_engine, captured_at)
...
def generate_run_dossier(
    ...
    *,
    ...
    if schema == "wang-run-case-v1":
        if include_pdf:
            raise DossierGenerationError("--pdf is a v2-only option")
    ...
    if schema == "wang-run-case-v2":
        ...
        try:
            return generate_multi_engine_dossier(
                case_path,
                output_directory,
